...
```{r slow}
tic()
hl_nsibts_slow <- icesDatras::getDATRAS("HL", survey = "NS-IBTS", 
                                        years = 1965:2025, quarters = 1:4)
toc()
```
...
```{r fast}
tic()
hl_nsibts_fast <- obus::dr_get_data_latin(recordtype = "HL", survey = "NS-IBTS",
                                          year = "1965:2025", quarter = "1:4") 
toc()
```
...
* DuckDB sets up the parquet file as if it were a table in a database, creating a view. A "view" is like a peek into the data — it doesn’t move or copy the underlying data but makes it look like a database table.
